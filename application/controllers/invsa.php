...
class Invsa extends CI_Controller {
	...
	public function add(){
		...
		if (strlen($data)>0) {
			 ...
			 $info1['id']    = 1;
			 $info1['billno']      = str_no('XS');
			 $info1['type']        = intval($data['transType']);
			 $info1['contactid']   = $data['buId'];
			 $info1['contactname'] = $contact['number'].' '.$contact['name'];
			 $info1['billdate']    = $data['date'];
			 //$info1['disamount']   = $data['disAmount'];
			 //$info1['disrate']     = $data['disRate'];
			 $info1['description'] = $data['description'];
			 $info1['totalamount'] = $data['totalAmount'];
			 $info1['totalqty']    = $data['totalQty'];
			 //$info1['amount']      = $info['type']==1?$data['amount']:-$data['amount'];  //折扣后金额
			 //$info1['rpamount']    = $info['type']==1?$data['rpAmount']:-$data['rpAmount'];  //已付款
			 //$info1['arrears']     = $info['type']==1?$data['arrears']:-$data['arrears'];  //欠款
			 //$info1['totalarrears'] = (float)$data['totalArrears'];
			 $info1['uid']         = $this->uid;
			 $info1['username']    = $this->name;
			 $info1['billtype']    = 2;
			 $info1['status']    = 0;
			 $this->db->trans_begin();
			 $invsaid = $this->mysql_model->db_inst(INVOI,$info1);

			 $info2['id']    = 2;
			 $info2['moneyid']      = str_no('CG');
			 //$info2['type']        = intval($data['transType']);
			 //$info2['contactid']   = intval($data['buId']);
			 $info2['contactname'] = $contact['number'].' '.$contact['name'];
			 $info2['billdate']    = $data['date'];
			// $info1['disamount']   = (float)$data['disAmount'];
			// $info1['disrate']     = (float)$data['disRate'];
			 //$info2['remark'] = $data['description'];
			 $info2['needmoney'] = (float)$data['totalAmount'];
			 //$info2['totalqty']    = (float)$data['totalQty'];
			 //$info1['amount']      = $info['type']==1?(float)$data['amount']:-(float)$data['amount'];  //折扣后金额
			 //$info1['rpamount']    = $info['type']==1?(float)$data['rpAmount']:-(float)$data['rpAmount'];  //已付款
			 //$info1['arrears']     = $info['type']==1?(float)$data['arrears']:-(float)$data['arrears'];  //欠款
			 //$info1['totalarrears'] = (float)$data['totalArrears'];
			// $info2['uid']         = $this->uid;
			 ...
			 //$info2['status']    = 0;
			 $this->db->trans_begin();
			 $this->mysql_model->db_inst(MONEY,$info2);
