...
class Basedata extends CI_Controller {
	...
	public function goods() {
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['safequantity'] = $row['safequantity'];
			$v[$arr]['safelevel'] = $row['safelevel'];
	...
	*/
	...
	public function goods_query() {
		...
		if (count($data)>0) {
			...
			$info['safequantity']  = (float)$data['safequantity'];
			$info['safelevel']     = (float)$data['safelevel'];
