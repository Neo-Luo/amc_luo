...
class Invoi extends CI_Controller {
	...
	public function turningto1() {
		...
		if (strlen($id)>0) {
			...
			$data['status'] = 1;
			$this->mysql_model->db_upd(INVOI,$data,'(id='.$id.')');
			$this->data_model->logs('确认入库');
	...
	public function outlist() {
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['userName']     = $row['username'];
			$v[$arr]['transTypeName']= $row['typename'];
			//$v[$arr]['contactaddress']= $row['contactaddress'];
