...
class Invpulack extends CI_Controller {
	...
	public function index(){
		$this->load->view('invpulack/index');
	}
	...
	public function add(){
		...
		if (strlen($data)>0) {
		    ...
		    $this->load->view('invpulack/add',$data);
		}
	}
	...
	public function edit(){
		...
		if (strlen($data)>0) {
			...
			if (count($data)>0) {
				$this->load->view('invpulack/edit',$data);
			} else {
			    $data['billno'] = str_no('CG');
			    $this->load->view('invpulack/add',$data);
			}
	...
	public function lists() {
		...
		$where = '';
		if (strlen($key)>0) {
		    $where .= ' and (id like "%'.$key.'%" or contactorderid like "%'.$key.'%" or goodsno like "%'.$key.'%")'; 
		}
		if (strlen($stt)>0) {
		    $where .= ' and contactdate>="'.$stt.'"'; 
		}
		if (strlen($ett)>0) {
		    $where .= ' and contactdate<="'.$ett.'"'; 
		}
		
		$offset = $rows * ($page-1);
		$data['data']['page']      = $page;
		$data['data']['records']   = $this->cache_model->load_total(LACK_INFO,'(1=1) '.$where);   //总条数
		$data['data']['total']     = ceil($data['data']['records']/$rows);    //总分页数
		$list = $this->cache_model->load_data(LACK_INFO,'(1=1) '.$where.' order by id desc limit '.$offset.','.$rows.'');  
		foreach ($list as $arr=>$row) {
			$v[$arr]['id']           = intval($row['id']);
			$v[$arr]['lackno']       = $row['lackno'];
			$v[$arr]['contactorderid']  = $row['contactorderid'];
			$v[$arr]['goodsname']= $row['goodsname'];
			$v[$arr]['goodsno']  = $row['goodsno'];
			$v[$arr]['lackqty']  = $row['lackqty'];
			$v[$arr]['contactdate']     = $row['contactdate'];
		}
	...
	public function export() {
		...
		if (strlen($id)>0) {
			...
			$this->load->view('invpulack/export',$data);
		}	
	}	
